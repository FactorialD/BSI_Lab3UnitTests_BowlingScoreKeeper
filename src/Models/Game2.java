...
public class Game2 {
    ...
    public void play() {
        ...
        for (int frameCounter = 0;!allPlayersEndedGame(); frameCounter++){
    ...
    private void printGameResult() {
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            System.out.print(spaces2.toString() + players.get(i).getName() + "║ ");
        ...
        if(players.get(0).getSummaryScoreOf(players.get(0).getFrames().size()-1).equals(players.get(1).getSummaryScoreOf(players.get(0).getFrames().size()-1))){
            ...
            System.out.println("║ "  + players.get(0).getName() + spaces2 + " wins!  ║" );
        }
    ...
    public boolean allPlayersEndedGame(){
    ...
    private void printScoreTable() {
        ...
        for (Player2 p: players
            ...
            int cellCount = 0;
            if(players.get(0).getFrames().size()>8){
                int b = 228;
            }
            ...
            cellCount++;
            //System.out.println("||added cCount for 1 roll of 1 frame. cell Count = " + cellCount + " ||");
            ...
            if((p.getFrames() == null || p.getFrames().size()-1 < 9) ||
                ...
                cellCount++;
                //System.out.println("||added cCount for 2 roll of 1 frame. cell Count = " + cellCount + " ||");
            }

            System.out.print("│");
            //second frame of 11 frame
            String last31attr;
            ...
            cellCount++;
            //System.out.println("||added cCount for 1 roll of 2 frame. cell Count = " + cellCount + " ||");
            ...
            if (cellCount > 2){
                ...
                cellCount++;
                //System.out.println("||added cCount for 2 roll of 2 frame. cell Count = " + cellCount + " ||");
            }

            //last cell of 10 frame
            //System.out.println("||cell Count = " + cellCount + " ||");
            if (cellCount > 2){
                //System.out.println("|| Last roll for 2 strikes ||");
            }else{
                System.out.print("│");
                String last51attr;
            ...
            printScoreTableForPlayer(p);

            System.out.println("╠════════════╬╬══╤══╬╬══╤══╬╬══╤══╬╬══╤══╬╬══╤══╬╬══╤══╬╬══╤══╬╬══╤══╬╬══╤══╬╬══╤══╤══╣");
            //System.out.println(p.getSummaryScores());
    ...
    private void printScoreTableForPlayer(Player2 player) {
        System.out.print("║            ║");
        for (int i = 0; i < 10; i++) {
    ...
    private void inputDownPills(Player2 player, Scanner in, int frameCounter) {


        ArrayList<Integer> res;

        res = new ArrayList<>();
        //System.out.println("    size of Frames: " + player.getFrames().size());
        if(player.getFrames().size() != 0){
            //System.out.println("    awaits for last roll: " + player.getFrames().get(player.getFrames().size()-1).awaitsForLastRoll());
        }

        if(player.getFrames().size() == 0 || player.isFirstRollInFrame()){
            ...
            ArrayList<Integer> leavedPills = player.getFrames().get(frameCounter).getLeavedPillsForFirstRoll();
        ...
        while(true){
            ...
            for (int pill:res
                ...
                if(res.size() > 10){
                    hasErrors = true;
                    System.out.println("Wrong number of pills! Try again.");

                }
                if(pill < 1 || pill > 10) {
                    hasErrors = true;
                    System.out.println("Wrong pill number! Try again.");
                }else{
                    if(player.getFrames().size() == 0 || player.isFirstRollInFrame()){
                        //break;
                    }else{
            ...
            if(hasNoRepeats){
                System.out.println("HAS NO REPEATS");
                break;
            }
        ...
        if(player.getFrames().size() == 0 || player.isFirstRollInFrame()){
            ...
            System.out.println("Player " + player.getName() + "additionally down pills:" + res.stream().map(Object::toString)
        ...
        player.makeRoll(res);
        System.out.println("|||| ->>> MAKED ROLL FOR" + player.getName() + "in " +frameCounter + " frame");

    }


    private void inputPlayerNames(int index, Scanner in) {
